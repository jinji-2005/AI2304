...
首先以 $32$ ms 分析窗和 $8$ ms 帧移计算谱特征，再按配置选择是否拼接一阶或二阶差分：
\begin{equation}
  \vec{g}_t =
  \begin{cases}
    \vec{u}_t, & \text{仅使用静态特征},\\
    [\vec{u}_t;\Delta \vec{u}_t], & \text{使用一阶差分},\\
    [\vec{u}_t;\Delta \vec{u}_t;\Delta^2 \vec{u}_t], & \text{使用一阶与二阶差分}.
  \end{cases}
\end{equation}
随后对每条语音做均值方差归一化
（CMVN），并使用上下文半宽 $C$ 进行拼接：
\begin{equation}
  \vec{f}_t^{(2)} = [\vec{g}_{t-C};\dots;\vec{g}_{t};\dots;\vec{g}_{t+C}].
\end{equation}
...
\begin{lstlisting}[language=python]
def extract_task2_features(waveform, feature_type, use_delta=True,
                           use_delta2=False, context=1):
    static = fbank_or_mfcc(waveform, win=32ms, hop=8ms, dim=K)
    feat = static
    if use_delta or use_delta2:
        delta = compute_delta(static)
    if use_delta:
        feat = concat([feat, delta], axis=1)
    if use_delta2:
        delta2 = compute_delta(delta)
        feat = concat([feat, delta2], axis=1)
    feat = cmvn(feat)
    return stack_context(feat, left=context, right=context)
\end{lstlisting}
...
语音后验概率。设上下文半宽为 $C$，静态特征维度为 $K$；仅使用静态特征时输入维数为
$D=(2C+1)K$，使用一阶差分时为 $D=(2C+1)2K$，同时使用一阶与二阶差分时为
$D=(2C+1)3K$。超参数配置见第~\ref{sec:task2-config} 节。
模型输出经 Sigmoid
函数转为语音后验概率，在 $\mathcal{D}_{\text{dev}}$ 上通过阈值搜索选择最优 $\tau$，并按实验配置对预测序列
做后处理。
...
def infer_task2(waveform, model, tau):
    ...
    pred = (scores >= tau)
    pred = median_smooth(pred, kernel=smooth_kernel)
...
\begin{itemize}
  ...
  \item 声学特征：采样率 $16$ kHz，帧长 $32$ ms，帧移 $8$ ms；基础特征维度 $K=40$，使用 CMVN，并比较 FBank/MFCC 及其差分组合；10 轮消融实验使用上下文半宽 $C=1$，最终 100 轮实验使用 $C=6$。
  \item 特征记号：特征名中的 $+\Delta$ 与 $+\Delta^2$ 分别表示拼接一阶与二阶差分；未标注差分时仅使用静态特征。
  \item GMM 配置：语音类使用 $4$ 个对角协方差高斯分量，非语音类使用单个高斯分量；模型输入先由训练集估计标准化器。
  \item DNN 配置：Network1 为两层隐藏层 MLP；10 轮消融实验使用隐藏层 $256/512$，最终最佳 100 轮配置也采用隐藏层 $256/512$。批量实验配置中的 Network2 在隐藏层后加入 BatchNorm 与 Dropout。
  \item 训练与判决：DNN 使用 Adam、学习率 $10^{-3}$、权重衰减 $10^{-5}$、batch size 为 $5210$，并采用 Focal Loss（$\alpha=0.25,\gamma=2.0$）。阈值在 $[0.1,0.9]$ 内以 $0.01$ 步长搜索；所有 DNN 实验统一使用长度为 $10$ 的平滑核。
\end{itemize}
...
在 10 轮训练设置下，Network1 + FBank+$\Delta+\Delta^2$ 取得较优整体性能。进一步将训练轮数延长至
$100$ 轮，上下文半宽调整为 $C=6$ 后，本实验对两个 Network1 配置进行单独比较，结果如表~\ref{tab:task2-dnn-100}。
...
\begin{center}
  \scriptsize
  \setlength{\tabcolsep}{2.4pt}
  \captionof{table}{Task2 DNN $\mathcal{D}_{\text{dev}}$ 100 轮训练结果（\%）}
  \label{tab:task2-dnn-100}
  \resizebox{\columnwidth}{!}{%
    \begin{tabular}{llrrrrrrrr}
      \toprule
      网络 & 特征 & Acc & Prec & Rec & $F_1$ & FAR & FRR & AUC & EER \\
      \midrule
      Network1 & FBank & 98.37 & 98.81 & 99.09 & 98.95 & 5.32 & 0.91 & 99.67 & 2.29 \\
      Network1 & FBank+$\Delta+\Delta^2$ & \best{98.54} & \best{99.02} & \best{99.12} & \best{99.07} & \best{4.37} & \best{0.88} & \best{99.74} & \best{2.04} \\
      \bottomrule
    \end{tabular}%
  }
\end{center}
...
\begin{center}
  ...
  \captionof{figure}{DNN 最终配置（Network1，FBank+$\Delta+\Delta^2$，100 轮训练）的训练损失曲线。}
...
\begin{center}
  ...
  \captionof{figure}{DNN 最终配置（Network1，FBank+$\Delta+\Delta^2$，100 轮训练）的 $\mathcal{D}_{\text{dev}}$ ROC 曲线。}
  \label{fig:task2-dnn-roc}
\end{center}

从表~\ref{tab:task2-dnn-100} 可以看到，在 100 轮训练和更大上下文设置下，FBank+$\Delta+\Delta^2$
在 Acc、Precision、Recall、$F_1$、FAR、FRR、AUC 与 EER 上均优于仅使用静态 FBank 的配置，因此本实验最终 DNN 配置采用
Network1 + FBank+$\Delta+\Delta^2$。与 GMM 相比，DNN 在整体准确率和 AUC 上均有明显提升，说明判别式模型能更充分利用频域动态特征。加入差分特征和
