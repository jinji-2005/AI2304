...
- [X] 选择频域特征（MFCC/PLP/FBank）
- [X] 选择模型（GMM 或 DNN）并实现训练/推理
- [X] 在 `dev` 集评估 Acc/AUC/EER
- [X] 生成 `task2/test_label.txt`

## 3. 报告（LaTeX）

- [X] 按模板填写 Task1：预处理与特征、算法描述、实验结果
- [X] 按模板填写 Task2：预处理与特征、算法描述、实验结果
- [X] 添加对比表（task1 vs task2）与关键参数表
- [X] 删除模板示例章节和提示文字
- [X] 生成最终 PDF（中文）
...
- 在 VAD 里的作用：
  ...
  - 二者结合通常比只用单一特征更稳。
- 短时频谱（Short-Time Spectrum）是什么：

  - 对每一帧做短时傅里叶变换（FFT/STFT），得到该帧在不同频率上的能量分布。
  - 若记第 $t$ 帧为 $x_t[n]$，其离散频谱可写为：
  - $$
    X_t[k]=\sum_{n=0}^{L-1}x_t[n]e^{-j2\pi kn/L}
    $$
  - 常用幅度谱或对数幅度谱作为统计基础，例如：
  - $$
    S_t=\frac{1}{K}\sum_{k=0}^{K-1}\log\left(1+|X_t[k]|\right)
    $$
  - 直觉：语音段在若干频带上会呈现更稳定、结构化的谱形；纯静音或某些噪声的谱形分布通常不同。
- 基频（Pitch / $F_0$）是什么：

  - 基频描述有声音段的“周期振动频率”，与听感上的“音高”直接相关。
  - 在时域中常用自相关法估计：在一段合理延迟范围内寻找自相关峰值位置 $\tau^\*$，再换算
    $F_0\approx f_s/\tau^\*$。
  - 直觉：浊音（如元音）通常有明显周期性，$F_0$ 更稳定；清音、噪声或静音往往不具备稳定基频。
- 在 VAD 里的作用：

  - 短时频谱可补充纯时域特征（energy、ZCR）对“频率结构”的感知能力。
  - 基频可强化对浊音语音段的识别，帮助区分“周期性语音”和“非周期噪声”。
  - 与能量/ZCR配合后，通常可降低部分边界误判和噪声误报。
- 实践提示：

  - 基频估计对无声帧不稳定，工程上常设置“相关峰值阈值”过滤低置信度帧。
  - 建议对频谱统计量与基频做标准化（如 z-score），再参与线性打分或阈值搜索。
  - 你代码里的“进频”可按“基频（Pitch/F0）”理解与实现。
...
### 6.8.1 BatchNorm 与其他归一化（Norm）方法

- BatchNorm（批归一化）是什么：

  - 对一个 mini-batch 内的特征做标准化，再用可学习参数恢复表达能力。
  - 常见形式：
  - $$
    \hat{x}=\frac{x-\mu_B}{\sqrt{\sigma_B^2+\epsilon}},\qquad y=\gamma \hat{x}+\beta
    $$
  - 其中，$\mu_B,\sigma_B^2$ 是当前 batch 统计量，$\gamma,\beta$ 是可学习参数。
- 训练和推理的差异：

  - 训练时：用当前 batch 的均值和方差。
  - 推理时：用训练过程中累计的 running mean / running var。
  - 这也是小 batch 下 BatchNorm 可能不稳的原因之一（统计量噪声大）。
- 在你当前 Task2（DNN）中的直觉作用：

  - 可减轻各层输入分布漂移，通常让训练更快、更稳。
  - 但如果 batch 太小或数据分布波动大，收益可能不稳定。
- 常见其他 Norm 方式：

  - LayerNorm（层归一化）：
    - 按“单个样本的特征维”归一化，不依赖 batch 统计量。
    - 小 batch、序列模型里通常更稳。
  - InstanceNorm（实例归一化）：
    - 按单样本单通道归一化，视觉任务常见。
    - 在语音帧级小 MLP 中不如 LayerNorm/BatchNorm 常用。
  - GroupNorm（组归一化）：
    - 在通道维分组后做归一化，介于 LayerNorm 与 InstanceNorm 之间。
    - 对 batch size 不敏感，常用于小 batch 场景。
  - RMSNorm：
    - 只做均方根缩放，不减均值，参数更少。
    - 在大模型中常见，任务小型网络里可选但不是首选。
- 与特征级标准化（CMVN / z-score）的关系：

  - CMVN/z-score 是“输入特征预处理”；
  - BatchNorm/LayerNorm 是“网络层内归一化”；
  - 两者不冲突，可叠加使用。
- 你这个项目的实操建议：

  - 若 `batch_size` 足够大（如几千以上），可先尝试 `BatchNorm1d`。
  - 若 batch 较小或不稳定，优先 `LayerNorm`。
  - 保留输入特征标准化（你已在特征中做 z-score/CMVN 思路），通常更稳。
